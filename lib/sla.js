...
function addChannelsToBridge(client, channel, dialed, bridge, extension) {
  ...
  function channelHold(event, channel) {
    ...
    isStation(extension, channel).then(function (isStation) {
      if (isStation) {
...
function originateInbound(client, inbound, bridge, extension) {
  ...
  getState(client, bridge.name).then(function(currentState) {
    ...
    if (currentState === 'NOT_INUSE') {
      ...
      // extension"
      var kickOutUser = Q.denodeify(inbound.continueInDialplan.bind(inbound));
      kickOutUser();
...
function originateOutbound(client, outbound, bridge, extension, trunkData) {
  ...
  getState(client, bridge.name).then(function (currentState) {
    if (currentState === 'INUSE' || currentState === 'RINGING' ||
      currentState === 'BUSY' || currentState === 'ONHOLD') {
      callInProgress = true;
    }
    if (!trunkData[0]) {
            ...
            .then(function (dialed) {
              return addChannelsToBridge(client, channel, dialed,
                bridge, extension).then(function(toFree) {
                  if (toFree === true) {
                    return Q.resolve('Extension ' + extensionName + ' freed!');
                  }
                });
            });
            } else {
              return originateOutbound(client, channel, bridge, extension,
                ...
                .then(function (dialed) {
                  return addChannelsToBridge(client, channel, dialed,
                    bridge, extension).then(function(toFree) {
                      if (toFree === true) {
                        return Q.resolve('Extension ' + extensionName +
                          ' freed!');
                      }
                    });
                })
