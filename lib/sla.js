 ...
 * @return {Q} - Q promise object
 */
function getTrunkData (configurationFile, extension, bridgeName) {
  var defer = Q.defer();
  dal.getSharedExtension(configurationFile, extension)
    .then(function(sharedExtension) {
    sharedExtension.getTrunk(bridgeName).then(function(trunk) {
      defer.resolve(trunk);
    })
 ...
 * @return {Q} Q promise object.
 */
module.exports = function(client, confFile, extension, inbound, bridgeName) {
  //This specifies what bridge number to use for SLA
  configurationFile = confFile;
  return getTrunkData(configurationFile, extension,
      bridgeName).then(function(trunk) {
