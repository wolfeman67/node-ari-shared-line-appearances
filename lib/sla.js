  ...
  function channelLeft(event, object) {
    if (isStation(extension, object.channel)) {
      if (object.bridge.channels.length === 1) {
        var hangupRemaining = Q.denodeify(client.channels.hangup.bind
            (client));
        hangupRemaining({channelId: object.bridge.channels[0]})
          .catch(function (err) {
...
function originateInbound(client, inbound, bridge, extension) {
  ...
  getState(client, bridge.name).then(function(currentState) {
    ...
    if (currentState !== states.INUSE && currentState !== states.BUSY &&
      ...
      var stations = extension.getAllStations();
      if (stations.length) {
        ...
        dialed.forEach(function (object) {
          ...
          object.channel.once('StasisStart', onStationEnteredStasis);
          object.channel.once('ChannelDestroyed', onStationHangup);
        });
...
function originateOutbound(client, trunkData, outbound, bridge) {
   ...
   * @param {Object} line - the line that has entered Stasis
   */
  function onTrunkEnteredStasis(event, line) {
    updateState(client, bridge.name, states.INUSE);
    outbound.removeListener('ChannelHangupRequest', onOutboundHangup);
    outbound.removeListener('ChannelDtmfReceived', onDtmfReceived);
    var answer = Q.denodeify(line.answer.bind(line));
    answer();
    defer.resolve(line);
  }
   ...
   */
  function onOutboundHangup(event, line) {
    updateState(client, bridge.name, states.IDLE);
    channelToDial.removeListener(
      'ChannelDestroyed', onTrunkHangup);
  ...
  function onDtmfReceived(event, channel) {
    ...
    switch (digit) {
      case '#':
        ...
        outbound.removeListener('ChannelHangupRequest', onEarlyOutboundHangup);
