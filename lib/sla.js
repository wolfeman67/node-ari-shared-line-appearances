...
function originateChannel(client, inbound, bridge) {
  ...
  var numToHangup;
  var dialed = [];
  dal.getAllStations(configurationFile, bridge.name)
    .then(function (stations) {
      var defer = Q.defer();
      if (stations.length) {
        stations.forEach(function(station, i, stations) {
        ...
        defer.reject(new CustomError('NoStations',
          'There are no stations in this bridge'));
      }
      return defer.promise;
    })
    .then(function(stations) {
      ...
      dialed.forEach(function (channel, i, dialed) {
        ...
        channel.once('StasisStart', function(event, line) {
        ...
        channel.once('ChannelDestroyed', function(event) {
          numToHangup -= 1;
          if(numToHangup === 0) {
 ...
 * @return {boolean} whether or not the bridgeName is in the conf. file.
 */
function isValid (configurationFile, bridgeName) {
  var defer = Q.defer();
  dal.findTrunk(configurationFile, bridgeName).then(function(position) {
    if (position !== -1) {
      defer.resolve(true);
    } else {
      defer.resolve(false);
 ...
 * @return {Q} Q promise object.
 */
module.exports = function(client, confFile, inbound, bridgeName) {
  //This specifies what bridge number to use for SLA
  configurationFile = confFile;
  return isValid(configurationFile, bridgeName).then(function(valid) {
    if(valid) {
      ...
      return Q.reject(new CustomError('SLASpecficationError',
            'Not a valid SLA specification: ' + bridgeName));
    }
  });
};
