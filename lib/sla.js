 ...
 * @return {Q} Q promise object.
 */
function addChannelsToBridge(inbound, dialed, bridge, trunkData) {
  var defer = Q.defer();
  var numInBridge = 0;
...
function originateChannel(client, inbound, bridge, trunkData) {
  ...
  function dialedChannelHungup(event) {
    ...
    if (allAreHungup(dialed)) {
      trunkToUse.busy = true;
      inbound.removeListener('ChannelHangupRequest', inboundHungup);
      return originateChannel(client, inbound, bridge, trunkData);
    }
  }
  ...
  if (!trunkToUse) {
    ...
    return Q.reject('ALL STATIONS BUSY!');
  }
  console.log(trunkToUse);
  var defer = Q.defer();
  var dialed = [];
 ...
 * @return {Q} - Q promise object
 */
function isInbound (configurationFile, extension, channel, dal) {
...
function getTrunkData (configurationFile, extension, dal) {
  console.log('GOOBY');
  return dal.getSharedExtension(configurationFile, extension)
    .then(function(sharedExtension) {
      console.log('DOLAN');
          ...
          return originateChannel(client, inbound, bridge, trunks, dal)
            .then(function (dialed) {
              ...
              if (typeof dialed !== 'string') {
                return addChannelsToBridge(inbound, dialed,
                  bridge, trunks).then(function(toFree) {
                    if (toFree === true) {
                      dal.clearSharedExtension(extension.name);
                      return Q.resolve('Application completed!');
                    } else {
                      return Q.resolve('Trunk freed');
                    }
                  });
              }
              else {
