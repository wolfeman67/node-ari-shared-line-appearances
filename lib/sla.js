...
function originateOutbound(client, outbound, bridge, extension, trunkData) {
    ...
    switch (digit) {
      case '#':
        ...
        var originate = Q.denodeify(channelToDial.originate.bind(
              channelToDial));
        originate({endpoint: 'SIP/' + toDial + '@'  + extension + '-' +
          trunkToUse.name, app: 'sla',
          appArgs: 'dialed',
          timeout: 10})
          .catch(function(err) {
            defer.reject(err);
  ...
  trunkData.some(function(trunk) {
    if (!trunk.user && !trunk.busy) {
      trunk.stations.forEach(function(station) {
        var re = regexp().find(station).toRegExp();
        if (re.test(outbound.name)) {
          trunkToUse = trunk;
          return true;
        } else {
          var nameDivided = outbound.name.split('/');
          if (re.test(nameDivided[0] + '/' + outbound.caller.number +
              '@' + nameDivided[1])) {
                trunkToUse = trunk;
                return true;
              }
 ...
 * @return {Q} Q promise object.
 */
module.exports = function(client, confFile, channel, extension) {
  return isInbound(confFile, extension, channel)
    .then(function (isInbound) {
                ...
                return originateInbound(client, channel, bridge, trunks)
                  .then(function (dialed) {
                    ...
                    if (typeof dialed !== 'string') {
                      ...
                      if (dialed === 'Inbound channel hungup') {
                        throw new CustomError('InboundHungup',
                          dialed);
                      }
                    }
                  });
                  ...
                  .then(function (dialed) {
                    if (typeof dialed !== 'string') {
                      ...
                      if (dialed === 'Call in progress') {
                        ...
                        throw new CustomError('EarlyHangup', dialed);
                      }
                    }
                  });
              }
