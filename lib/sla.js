...
function originateInbound(client, inbound, bridge, extension) {
  ...
  function dialedEnteredStasis(event, line) {
    ...
    updateState(client, bridge.name, states.IN_USE);
    defer.resolve(line);
  }
   ...
   * @param {Object} channel - the channel object that is attempting
   *    to be found.
...
function originateOutbound(client, outbound, bridge, extension, trunkData) {
   ...
   */
  function dialedEnteredStasis(event, line) {
    console.log('Trunk ' + line.name + ' accepted the call.');
    updateState(client, bridge.name, states.IN_USE);
    ...
    channelsToDial.forEach(function(chan) {
        chan.removeListener('ChannelDestroyed', dialedChannelHungup);
        if (chan.id !== line.id) {
   ...
   * Function that gets called when a dialed channel hangs up.
   * @param {Object} event - the event object related to this hang up.
   * @param {Object} line - dialed channel that hungup.
   */
  function dialedChannelHungup(event, line) {
    var index = channelsToDial.indexOf(line);
    channelsToDial.forEach(function(channel, index) {
  ...
  /**
   * Function that originates a call to a trunk for outbound dialing.
   * @param {String} trunk - trunk to originate the call to.
   */
  function originateToTrunk(trunk) {
    ...
    console.log('Originating to trunk ' + trunk);
    channelsToDial[index].once('StasisStart', dialedEnteredStasis);
    channelsToDial[index].once('ChannelDestroyed', dialedChannelHungup);
  }
  ...
  getState(client, bridge.name).then(function (currentState) {
    ...
    if (!trunkData[0]) {
      ...
      defer.reject(new customError.CustomError('ExtensionBusy',
          'Call in progress'));
    }
  });
  return defer.promise;
...
module.exports = function(client, confFile, channel, extensionName) {
  return dal.getSharedExtension(confFile, extensionName)
    .then(function(extension) {
      return isStation(extension, channel)
      .then(function (isStation) {
        return getTrunkData(extension)
        .then(function(trunks) {
          ...
          .then(function() {
            ...
            if (!isStation) {
              return originateInbound(client, channel, bridge, extension)
            .then(function (dialed) {
              return addChannelsToBridge(client, channel, dialed,
                bridge, extension).then(function(toFree) {
                  if (toFree === true) {
              ...
              .catch(function (err) {
                if (err.name === 'ExtensionBusy') {
                  return addChannelsToBridge(client, channel, null, bridge,
                    extension).then(function(toFree) {
                      if (toFree === true) {
                        return Q.resolve('Application completed!');
                      }
                    });
  ...
  .catch(function (err) {
    if (err.name !== 'InboundHungup' || err.name !== 'OutboundHungup' ||
      err.name !== 'OutboundHungupEarly' || err.name !== 'ExtensionOccupied') {
