  ...
  return dal.getSharedExtension(configurationFile, extension)
    .then(function(sharedExtension) {
      return sharedExtension.getAllStations().then(function(stations) {
        ...
        if (stations.indexOf(channel.name) !== -1) {
          console.log('Koopa Klown Kar');
          return Q.resolve(true);
        } else {
          console.log('Lot\'ts of spaghetti!');
...
module.exports = function(client, confFile, channel, extension) {
  return isInbound(confFile, extension, channel)
    .then(function (isInbound) {
      if (isInbound) {
        console.log(channel.name);
        return Q.resolve("Pingas");
      }
    });
};
function inBoundDialing(client, confFile, channel, extension) {
